...
beforeEach(() => {
  ...
  databaseMocks.readDatabase = {};
});

describe("transit directory repository", () => {
  it("uses the built-in directory when no read database is configured", async () => {
    databaseMocks.readConfigured.mockReturnValue(false);
  ...
  it("reads only published entries in rank order", async () => {
    databaseMocks.readConfigured.mockReturnValue(true);
    const orderBy = vi.fn().mockResolvedValue([input]);
    const where = vi.fn(() => ({ orderBy }));
    const from = vi.fn(() => ({ where }));
    databaseMocks.readDatabase.select = vi.fn(() => ({ from }));
