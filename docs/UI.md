...
全球价格工作区与产品/套餐页提供原生地区下拉框和两地差额，控件至少 44px 高。结果链接以 `provider`、`plan` 参数和 `#compare?from=...&to=...` 保存选择；这些选择不新增独立索引页。复制失败提供可访问的结果链接，报价更新延迟与汇率日期差异保持可见。无 JavaScript 时保留默认价格和来源，选择、复制及展开历史需要 JavaScript。

历史面板和价格变化页区分加载中、真实空记录与暂时不可用；不能把“未查到”写成“从未变价”。价格变化以行式列表展示日期、产品、前后原币金额和来源，RSS/CSV/JSON 提供独立数据入口。
