...
优先使用 **QQ 邮箱 SMTP 授权码**做临时配置。

理由：

- 开通简单，不需要先购买或验证域名；
- 国内收件箱测试方便；
- 项目当前邮件量小，足够验证订阅确认、退订和管理员告警。
...
当前 VPS 的 443 已被 v2ray 使用。配置站点 HTTPS 前，需要先处理 443 共存问题，
不能直接让 Nginx 或 Certbot 抢占端口。
