...
Kimi 会员采集按官方表头选择月付价格表，兼容人民币地区页与官方 `kimi.ai` 中文国际页；
当前生产采集使用实际返回价格的 `https://www.kimi.ai/zh-hans/help/membership/membership-pricing`，
国际页返回 USD 时保留原币种，并由现有 FX 快照生成人民币参考，不把美元金额伪装成人民币。

Huawei MaaS 的官方表当前核验基线为 9 个指定模型、27 个报价和 cached/input/output
三类价格；缺少指定模型或价格类型时仍报结构变化，不静默接受缩水页面。
