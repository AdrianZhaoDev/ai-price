...
采集故障。计划任务通过 `--trigger=scheduled` 标记；人工单来源复核仍记为 `manual`，
不得被误计为连续计划失败。每轮失败会计算这两个阈值并按来源去重发送管理员告警；日报
还必须主动检查达到 8 小时但采集期间未再次触发的开放错误。成功采集会清零连续失败并
解决该来源的开放错误。日报必须重新读取最近完整运行，不得把 219/219 基线当作当日结果。
